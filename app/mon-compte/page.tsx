    ...
    return (
      <div className="max-w-3xl mx-auto px-6 py-14">
        <div className="bg-brand-surface border border-brand-border rounded-2xl p-8 text-center text-brand-ink-faint text-sm">
          Connectez Supabase pour activer les comptes.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-14 flex flex-col gap-10">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex flex-col gap-2.5">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <VerifiedBadge verified={profile.identity_verified} />
            {profile.email && (
        ...
        <div className="flex items-center gap-2">
          ...
          <SignOutButton className="h-10 px-4 rounded-xl border border-brand-border text-sm font-semibold hover:bg-brand-bg transition" />
        </div>
      </header>
      ...
      <section className="flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="font-display font-bold text-lg">
            Mes annonces {listings.length > 0 && `(${listings.length})`}
          </h2>
          <Link
            href="/publier"
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div>
                    ...
                    <div className="text-[13px] text-brand-ink-faint mt-0.5">
                      {l.type === "don" ? "Don" : "Demande"} · {l.wilaya} ·{" "}
                      {relativeTimeFr(l.created_at)}
                    </div>
                  </div>
                  <StatusPill
                    label={LISTING_STATUS_LABEL[l.status]}
                    active={l.status === "active"}
                  />
                </div>
                <StatusActions table="listings" id={l.id} status={l.status} />
              </div>
            ))}
          </div>
        )}
      ...
      <section className="flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="font-display font-bold text-lg">
            Mes trajets {trips.length > 0 && `(${trips.length})`}
          </h2>
          <Link
        ...
        {trips.length === 0 ? (
          <div className="bg-brand-surface border border-brand-border rounded-2xl p-7 text-center text-brand-ink-faint text-sm">
            Vous n&apos;avez annoncé aucun trajet.
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {trips.map((t) => (
              ...
              >
                ...
                <StatusActions table="trips" id={t.id} status={t.status} />
              </div>
            ))}
          </div>
        )}
      ...
      <section className="flex flex-col gap-4 border-t border-brand-border pt-8">
        <h2 className="font-display font-bold text-lg">Mes données</h2>
        ...
        <div className="flex gap-3 px-4 py-4 bg-brand-surface border border-brand-border rounded-xl">
          ...
          <p className="text-xs text-brand-ink-soft leading-relaxed">
            Retirer une publication la fait disparaître du site tout en la
            conservant en base. La supprimer l&apos;efface définitivement. Pour
            la fermeture complète de votre compte, écrivez-nous — voir{" "}
            <Link href="/confidentialite" className="underline underline-offset-2">
              la page sur vos données
            </Link>
            .
          </p>
        </div>

        <DeleteEverything count={total} />
      </section>
    </div>
  );
}
